...
import type { ShoppingProduct } from '@/sanity/lib/queries'
...
// NOTE: keep this instruction free of any literal "$word" — Sanity's agent
// API treats every $word in the instruction text itself as an unresolved
// template variable (distinct from $query below, which is a real declared
// variable). A "$100" example here previously 400'd every single call with
// "missing instructionParams" until it was removed.
...
// on any failure (network, plan not AI-enabled, timeout, malformed output);
// the caller is expected to fall back to a deterministic response rather
// than let a live demo hang on an external model call.
...
export async function runFilteredSearch(filters: SearchFilters): Promise<ShoppingProduct[]> {
  const params = {
